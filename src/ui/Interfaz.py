...
def obtener_info_sistema():
    try:
        ...
        messagebox.showerror(
            "Error", f"No se pudo obtener la información de la red: {e}"
        )
...
def move_files():
    ...
    if not os.path.exists(source_path):
        messagebox.showwarning(
            "Advertencia",
            f"La ruta de origen '{source_path}' no existe. Seleccione una manualmente.",
        )
    ...
    try:
        ...
        if final_folder:
            ...
            messagebox.showinfo(
                "Sin cambios", "No se encontraron archivos .dat para mover."
            )
    except Exception as e:
...
def abrir_testeo():
    ...
    if not os.path.exists(ruta_programa):
        messagebox.showerror(
            "Error", f"No se encuentra el programa en la ubicación: {ruta_programa}"
        )
        return
...
def run_app():
    """
    Envuelve la configuración de la ventana y ejecuta el loop principal.
    """
    ctk.set_appearance_mode("dark")  # Tema oscuro
    ctk.set_default_color_theme("blue")  # Tema moderno

    global root, pages, current_page
    global label_equipo, label_ip, label_via, label_puertos
    global label_antena_ip, label_potencia, label_tag1_info, label_tag2_info
    global source_var, dest_var, potencia_entry
    ...
    label_equipo = ctk.CTkLabel(
        frame_info, text="Equipo: Cargando...", anchor="w", text_color="white"
    )
    label_equipo.pack()
    label_ip = ctk.CTkLabel(
        frame_info, text="IP: Cargando...", anchor="w", text_color="white"
    )
    label_ip.pack()
    label_via = ctk.CTkLabel(
        frame_info, text="Vía: Cargando...", anchor="w", text_color="white"
    )
    label_via.pack()
    ctk.CTkButton(
        frame_info,
        text="Actualizar Info",
        command=obtener_info_sistema,
        fg_color="#9B30FF",
    ).pack(pady=10)
    ...
    ctk.CTkLabel(
        frame_puertos,
        text="Puertos COM Activos",
        font=("Arial", 14, "bold"),
        text_color="black",
    ).pack(pady=5)
    label_puertos = ctk.CTkLabel(
        frame_puertos, text="Cargando...", anchor="w", text_color="black"
    )
    label_puertos.pack()
    ctk.CTkButton(
        frame_puertos,
        text="Detectar Puertos",
        command=detectar_puertos_com,
        fg_color="darkblue",
    ).pack(pady=10)
    ...
    ctk.CTkLabel(
        frame_testeo,
        text="Ingrese al Testeo",
        font=("Arial", 14, "bold"),
        text_color="white",
    ).pack(pady=5)
    ctk.CTkButton(
        frame_testeo, text="Abrir Testeo", command=abrir_testeo, fg_color="darkorange"
    ...
    ctk.CTkButton(
        ...
        command=lambda: dest_var.set(filedialog.askdirectory()),
    ).pack(pady=5)
    ctk.CTkButton(
        frame_archivos, text="Mover Archivos", command=move_files, fg_color="darkgreen"
    ).pack(pady=10)
    ...
    ctk.CTkLabel(
        frame_antena,
        text="Configuración de Antena",
        font=("Arial", 14, "bold"),
        text_color="white",
    ).pack(pady=5)
    label_antena_ip = ctk.CTkLabel(
        frame_antena, text="IP Antena: Cargando...", anchor="w", text_color="white"
    )
    label_antena_ip.pack()
    label_potencia = ctk.CTkLabel(
        frame_antena, text="POTENCIA: Cargando...", anchor="w", text_color="white"
    )
    label_potencia.pack()
    ...
    ctk.CTkButton(
        frame_antena,
        text="Actualizar Potencia",
        command=modificar_potencia,
        fg_color="#4682B4",
    ).pack(pady=5)
    ctk.CTkButton(
        frame_antena,
        text="Mostrar Datos",
        command=actualizar_datos_antena,
        fg_color="#4169E1",
    ).pack(pady=5)

    # Panel: Última Actualización de Listas de Tags
    frame_tags = ctk.CTkFrame(
        page2,
        fg_color="#FFD700",
        corner_radius=10,
        border_width=2,
    ...
    title_label = ctk.CTkLabel(
        frame_tags,
        text="Última Actualización de Tags",
        font=("Arial", 14, "bold"),
        text_color="black",
    )
    title_label.grid(row=0, column=0, columnspan=2, pady=10, sticky="n")

    label_tag1_name = ctk.CTkLabel(
        frame_tags,
        text="LMTAGS_AUBASA.DAT:",
        font=("Arial", 12, "bold"),
        text_color="black",
        anchor="w",
    )
    ...
    label_tag1_info = ctk.CTkLabel(
        frame_tags, text="Archivo no encontrado", anchor="w", text_color="black"
    )
    label_tag1_info.grid(row=2, column=0, sticky="w", padx=5, pady=5)

    label_tag2_name = ctk.CTkLabel(
        frame_tags,
        text="LMTAGSPAT_AUBASA.DAT:",
        font=("Arial", 12, "bold"),
        text_color="black",
        anchor="w",
    )
    ...
    label_tag2_info = ctk.CTkLabel(
        frame_tags, text="Archivo no encontrado", anchor="w", text_color="black"
    )
    label_tag2_info.grid(row=4, column=0, sticky="w", padx=10, pady=5)

    ctk.CTkButton(frame_tags, text="Actualizar", command=actualizar_info_tags).grid(
